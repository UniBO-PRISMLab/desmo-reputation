...
class Indexer:
    def __init__(self, trusted=True, owner: int = 1) -> None:
    ...
    def selectProducers(self, epoch, policy=constants.INDEXER_POLICY):
        ...
        if policy == constants.INDEXER_POLICY_RANDOM:
            ...
            selected = np.random.choice(
                self.indexed_sources, num_producers_to_select, p=(weights / np.sum(weights)), replace=False
            )
        else:
    ...
    def print_self(self, verbose=False):
        ...
        if not verbose:
            ...
            print(
                ...
                + str(round(self.reputation * 100, 1))
                + " \n\t Last Request: "
                + str(self.last_request)
                + " \n\t Indexed: "
                + str(self.indexed_sources)
                + " \n\t Last Selected: "
                + str(self.selected_producers)
            )  # str([item for item in self.indexed_sources if (Producers[item].last_request == self.last_request)]))
