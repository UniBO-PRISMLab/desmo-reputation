...
FAV_CHAIN_arr = [-1] # XXX
ARTIFICIAL_DELAY_arr = [0.0, 10.0, 30.0, 60.0, 300.0, 900.0, 1800.0, 3600.0, 7200.0]


# COST AND TIME PARAMETER
COST_TIME_PARAM_arr = [0, 0.25, 0.5, 0.75, 1.0]
COST_TIME_PARAM_arr = [0.5] #XXX

ARRIVAL_RATE_arr = ['uniform']

# FILE
FILE_OUT_path = "results-distributed"
FILE_OUT_path = "results-multichain"
FILE_OUT_prefix = "REP"
...
counter = 0
total = REPETITIONS * len(TRUTH) * len(REPUTATION_MIN_arr) * len(COST_TIME_PARAM_arr) * len(RATIO_MALICIOUS_arr)* len(ALGO)*len(ARRIVAL_RATE_arr)*len(FAV_CHAIN_arr)*len(RATIO_MALICIOUS_ORACLES_arr) * len(ARTIFICIAL_DELAY_arr)
                            ...
                            for arrival_rate in ARRIVAL_RATE_arr:
                                for truth in TRUTH:
                                    for fav_chain in FAV_CHAIN_arr:
                                        for artificial_delay in ARTIFICIAL_DELAY_arr:   
                                            ...
                                            if OVERWRITE or not os.path.exists(filepath):
                                                ...
                                                process = Popen([
                                                    ...
                                                    "--artificial_delay", str(artificial_delay)
                                                ])
